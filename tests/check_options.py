...
if os.path.isfile(constants.default_log_file):
    os.system("mv " + constants.default_log_file + " " +
              constants.default_log_file + ".backup")
...
def test_unregister():
    unregister = subprocess.Popen([constants.app_name + " --unregister"],
...
def test_register():
    register = subprocess.Popen([constants.app_name + " --register"],
                                stdout=subprocess.PIPE, shell=True)
    register_output, err = register.communicate()
    print register_output
    if (re.search("Successfully registered", register_output) or
            re.search("already been registerd", register_output)) is not None:
        pass
    else:
        print err
...
def test_option_disable_schedule():
    if os.path.isfile("/etc/cron.daily/" + constants.app_name):
        os.system("cp /etc/cron.daily/" + constants.app_name +
                  " /etc/cron.daily/" + constants.app_name + ".backup")
    ...
    if os.path.isfile("/etc/cron.daily/" + constants.app_name + ".backup"):
        os.system("mv /etc/cron.daily/" + constants.app_name +
                  ".backup /etc/cron.daily/" + constants.app_name)
...
def test_gpg_verification_check():
    check_gpg = subprocess.Popen([constants.app_name],
                                 stdout=subprocess.PIPE, shell=True)
    ...
    try:
        mylogger.debug("checking if the gpg check fails on \
                       modifying .cache.json")
        os.system("cp " + os.path.join(constants.default_conf_dir,
                                       ".cache.json") + " " +
                  os.path.join(constants.default_conf_dir,
                               ".cache.json.backup"))
        with open(os.path.join(constants.default_conf_dir, ".cache.json"),
                  "a") as f:
            f.write("#changing .cache.json")

        os.system("cp " + constants.default_conf_file + " " +
                  constants.default_conf_file + ".backup")
        with open(constants.default_conf_file, "r") as f:
            auto_update_change = f.read().replace("#auto_update=True",
                                                  "auto_update=False")
        with open(constants.default_conf_file, "w") as f:
            f.write(auto_update_change)

        gpg = subprocess.Popen([constants.app_name], stdout=subprocess.PIPE,
        ...
        if re.search(gpg_regex, gpg_err) is not None:
            ...
            pytest.fail("should not be able to upload successfully \
                        without gpg verification")

    except Exception as e:
        print e
        os.system("rm " + os.path.join(constants.default_conf_dir,
                                       ".cache.json"))
        os.system("mv " + os.path.join(constants.default_conf_dir,
                                       ".cache.json.backup") + " " +
                  os.path.join(constants.default_conf_dir, ".cache.json"))
        os.system("rm " + constants.default_conf_file)
        os.system("mv " + constants.default_conf_file + ".backup " +
                  constants.default_conf_file)
        print "reverted changes made to .cache.json and " +  \
            constants.default_conf_file_name
        pytest.fail("should not be able to upload successfully \
                    without gpg verification")
    ...
    os.system("mv " + os.path.join(constants.default_conf_dir,
                                   ".cache.json.backup") + " " +
              os.path.join(constants.default_conf_dir, ".cache.json"))
    print "Reverting changes made to " + constants.default_conf_dir + \
        ".cache.json and " + constants.default_conf_file
    os.system("rm " + constants.default_conf_file)
    os.system("mv " + constants.default_conf_file + ".backup " +
              constants.default_conf_file)
...
def test_gpg_verification_disabled_check():
    gpg_disabled = subprocess.Popen([constants.app_name + " --no-gpg"],
                                    stdout=subprocess.PIPE, shell=True)
    gpg_check_text, err = gpg_disabled.communicate()
    if re.search("GPG VERIFICATION DISABLED", gpg_check_text) is not None:
        pass
        mylogger.debug("--no-gpg option disables \
                       gpg verification successfully")
    else:
...
def test_blacklist_functionality_with_keep_archive():
    if os.path.isfile(constants.collection_remove_file):
        os.system("mv " + constants.collection_remove_file + " " +
                  constants.collection_remove_file + ".backup")
    os.system("cp ./tests/data/" + constants.collection_remove_file_name +
              " " + constants.collection_remove_file)

    remove_conf = subprocess.Popen([constants.app_name + " --keep-archive"],
    ...
    if os.path.isfile(constants.collection_remove_file + ".backup"):
        os.system("mv " + constants.collection_remove_file + ".backup " +
                  constants.collection_remove_file)

    if os.path.isfile("/tmp/%s/insights_commands/lspci" % archive_directory):
        pytest.fail("The command '/sbin/lspci' in the " +
                    constants.collection_remove_file_name +
                    " did not get blacklisted")
    else:
        print "The command '/sbin/lspci' from " + \
            constants.collection_remove_file_name + \
            " got blacklisted(not executed) - VERIFIED"
    ...
    if os.path.isfile("/tmp/%s/etc/hosts" % archive_directory):
        pytest.fail("The file 'etc/ssh/hosts' from " +
                    constants.collection_remove_file_name +
                    " did not get blacklisted")
    else:
        print "The file 'etc/hosts from " + \
            constants.collection_remove_file_name + \
            " got blacklisted - VERIFIED"
...
def test_machine_id_verification():
    if os.path.isfile(constants.machine_id_file):
        machine_id = subprocess.Popen(["cat " + constants.machine_id_file],
                                      stdout=subprocess.PIPE,
                                      stderr=subprocess.PIPE, shell=True)
        machineid, err = machine_id.communicate()
        print "\n'" + constants.machine_id_file + "' is present as: %s" \
            % machineid
    else:
...
def test_no_upload_option():
    no_upload = subprocess.Popen([constants.app_name + " --no-upload"],
                                 stdout=subprocess.PIPE, shell=True)
    no_upload_output, err = no_upload.communicate()
    print no_upload_output
    tar_regex = re.compile(r".*tar\.gz.*")
    if re.search(tar_regex, no_upload_output) is not None and \
            re.search("Upload completed successully!",
                      no_upload_output) is None:
        tar_file = no_upload_output.split("See Insights data in ")[1]
        print tar_file
    else:
        print err
        pytest.fail("The upload was successfull despite of the \
                    option --no-upload")
...
def test_no_tar_file_option():
    no_tar_file = subprocess.Popen([constants.app_name + " --no-tar-file"],
                                   stdout=subprocess.PIPE, shell=True)
    no_tar_output, err = no_tar_file.communicate()
    print no_tar_output
    tar_regex = re.compile(r".*tar\.gz.*")
    if re.search(tar_regex, no_tar_output) is None and \
            re.search("Upload completed successully!", no_tar_output) is None:
        no_tar_dir = no_tar_output.split("See Insights data in ")[1]
        print no_tar_dir
    else:
        print err
        pytest.fail("The tar file should not have been created with usage of \
                    option --no-tar-file")
...
def test_version_option():
    version = subprocess.Popen([constants.app_name + " --version"],
                               stdout=subprocess.PIPE, shell=True)
...
def test_update_collection_rules():
    update_collection = subprocess.Popen([constants.app_name +
...
def test_compressor_option():
    compressor = subprocess.Popen([constants.app_name +
                                   " --compressor=bzip2 --no-upload"],
                                  stdout=subprocess.PIPE, shell=True)
    compressor_output, err = compressor.communicate()
    print compressor_output
    compressor_regex = re.compile(r".*tar\.bzip2.*")
    if re.search(compressor_regex, compressor_output) is not None and \
            re.search("Upload completed successully!",
                      compressor_output) is None:
...
def test_custom_config():
    custom_config = subprocess.Popen([constants.app_name +
                                      " --conf=./test/data/test-custom.conf"],
                                     stdout=subprocess.PIPE, shell=True)
